...
# read features list (variable names)
...
meltData<-melt(data,id.vars=c("ActLabels","SubjLabels"),measure.vars=c(grep("mean()",features[,2],fixed=T),grep("std()",features[,2],fixed=T)))
